...
## 2024-04-13 - docker compose v2 container naming convention

**Learning:** Docker Compose v2 changes the default container naming convention to
separate components using hyphens rather than underscores
(`<project-name>-<service-name>-<index>`). **Action:** Update any scripts referencing
container names generated by Docker Compose to use hyphens (e.g., `dinar-odoo-1` instead
of `dinar_odoo_1`) to avoid "No such object" errors during `docker inspect` or
`docker exec`.
